...
void Chusky::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO1, m_firstInsCombobox);
	DDX_Control(pDX, IDC_COMBO2, m_secondInsCombobox);
	DDX_Control(pDX, IDC_INS_SELL_EDIT1, m_firstSellEdit);
	DDX_Control(pDX, IDC_INS_BUY_EDIT1, m_firstBuyEdit);
	DDX_Control(pDX, IDC_INS_SELL_EDIT2, m_secondSellEdit);
	DDX_Control(pDX, IDC_INS_BUY_EDIT2, m_secondBuyEdit);
	DDX_Control(pDX, IDC_INS_SELL_DELTA_EDIT, m_sellDeltaEdit);
	DDX_Control(pDX, IDC_INS_BUY_DELTA_EDIT, m_buyDeltaEdit);
}
...
BEGIN_MESSAGE_MAP(Chusky, CDialogEx)
	...
	ON_EN_CHANGE(IDC_INS_BUY_EDIT1, &Chusky::OnEnChangeInsBuyEdit1)
	ON_BN_CLICKED(IDC_SELL_DELTA_BUTTON, &Chusky::OnBnClickedSellDeltaButton)
	ON_BN_CLICKED(IDC_BUY_DELTA_BUTTON, &Chusky::OnBnClickedBuyDeltaButton)
END_MESSAGE_MAP()
...
void Chusky::setMoney(){
	CUstpFtdcQryInvestorAccountField qryInvestorAccount;	
	strcpy(qryInvestorAccount.BrokerID, BrokerID);
	strcpy(qryInvestorAccount.InvestorID, "01937586");
...
void Chusky::setPosition(char *InstrumentID){
	...
	strcpy(QryUserInvestorPosition.InvestorID, "01937586");
...
void Chusky::OnBnClickedStrategyButton()
{
	...
	if (mark == 1){
		...
		strcpy(para.InvestorID, "01937586");
...
void Chusky::OnEnChangeInsBuyEdit1()
{
...
void Chusky::OnBnClickedSellDeltaButton()
{
	// TODO: 在此添加控件通知处理程序代码
	CUstpFtdcInputOrderField order;
	strcpy(order.BrokerID, BrokerID);
	...
	strcpy(order.UserID, UserID);
	UpdateData(TRUE);
	int nSelOfName = m_secondInsCombobox.GetCurSel();
	strcpy(order.InstrumentID, contracts[nSelOfName]);
	order.Direction = USTP_FTDC_D_Sell;
	if (IsDlgButtonChecked(IDC_OPEN_RADIO)) order.OffsetFlag = '0';
	if (IsDlgButtonChecked(IDC_CLOSE_RADIO)) order.OffsetFlag = '1';
	...
	sprintf(order.UserOrderLocalID, "%012d", g_UserOrderLocalID);
	order.OrderPriceType = USTP_FTDC_OPT_LimitPrice;
	order.LimitPrice = InsData[nSelOfName].BidPrice1;
	order.TimeCondition = USTP_FTDC_TC_GFD;
	pTraderApi -> ReqOrderInsert(&order, g_UserOrderLocalID);

	nSelOfName = m_firstInsCombobox.GetCurSel();
	strcpy(order.InstrumentID, contracts[nSelOfName]);
	order.Direction = USTP_FTDC_D_Buy;
	if (IsDlgButtonChecked(IDC_OPEN_RADIO)) order.OffsetFlag = '0';
	if (IsDlgButtonChecked(IDC_CLOSE_RADIO)) order.OffsetFlag = '1';
	...
	sprintf(order.UserOrderLocalID, "%012d", g_UserOrderLocalID);
	order.OrderPriceType = USTP_FTDC_OPT_LimitPrice;
	order.LimitPrice = InsData[nSelOfName].AskPrice1;
	order.TimeCondition = USTP_FTDC_TC_GFD;
	pTraderApi -> ReqOrderInsert(&order, g_UserOrderLocalID);
}


void Chusky::OnBnClickedBuyDeltaButton()
{
	// TODO: 在此添加控件通知处理程序代码
	CUstpFtdcInputOrderField order;
	strcpy(order.BrokerID, BrokerID);
	...
	strcpy(order.UserID, UserID);
	UpdateData(TRUE);
	int nSelOfName = m_firstInsCombobox.GetCurSel();
	strcpy(order.InstrumentID, contracts[nSelOfName]);
	order.Direction = USTP_FTDC_D_Sell;
	if (IsDlgButtonChecked(IDC_OPEN_RADIO)) order.OffsetFlag = '0';
	if (IsDlgButtonChecked(IDC_CLOSE_RADIO)) order.OffsetFlag = '1';
	...
	nSelOfName = m_secondInsCombobox.GetCurSel();
	strcpy(order.InstrumentID, contracts[nSelOfName]);
	order.Direction = USTP_FTDC_D_Buy;
	if (IsDlgButtonChecked(IDC_OPEN_RADIO)) order.OffsetFlag = '0';
	if (IsDlgButtonChecked(IDC_CLOSE_RADIO)) order.OffsetFlag = '1';
